...
export async function GET(
  ...
  if (requiredOwnerKey && !hasSignedInOwnerAccess) {
    ...
    if (accountApiAuth.ownerKey !== requiredOwnerKey) {
      return NextResponse.json(
        { error: { code: "FORBIDDEN", message: "This API key cannot access the requested output" } },
        { status: 403 }
      );
    }
    ...
    if (!hasAccountApiKeyScope(accountApiAuth, "generations:read")) {
      return NextResponse.json(
        { error: { code: "FORBIDDEN", message: "This API key cannot download job outputs" } },
        { status: 403 }
      );
    }
    ...
    const rateLimitViolation = await enforceRateLimits([
      {
        bucket: "generation-output-download:api-key",
      ...
      {
        bucket: "generation-output-download:ip-safety",
    ...
    const rateLimitViolation = await enforceRateLimits([
      {
        bucket: "generation-output-download:anonymous-ip",
        identifiers: [clientIp],
